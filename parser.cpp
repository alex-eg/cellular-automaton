...
void Settings::add_to_section(std::string sec_name, std::string key,
                            std::string value) {
...
std::string Settings::find(std::string item, std::string section) {
    for (auto &kv_section : sections) {
        for (auto &kv_parameter : kv_section.second) {
            if (kv_parameter.first == item) {
                return kv_parameter.second;
            }
        }
    }
    return std::string("");
...
Settings &Settings::operator=(const Settings &right) {
    ...
    for (auto &kv_section : right.sections) {
        for (auto &kv_parameter : kv_section.second) {
            sections[kv_section.first][kv_parameter.first] =
                kv_parameter.second;
        }
    }
    return *this;
    ...
    while (!inifile.eof()) {
        ...
        if (line[0] == '[') {
            if (line[line.length() - 1] != ']') {
                throw("Malformed section name");
            }
            current_section = line.substr(1, line.length() - 2);
            std::cout << "<<section changed: >>" << current_section
                      << std::endl;
            continue;
        }
